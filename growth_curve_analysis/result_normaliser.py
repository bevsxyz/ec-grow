...
def extract_n_write_OD_values(f):                  # Extracts the OD values from all the input files and neatly prints
                                                   # them on a csv file
    global file_order, LastIteration               # Declares the global parameters
...
def reorganise_data(ftbc):                         # Reorganises the data in a temporary tsv file
    global file_order, LastIteration               # Declares the global values the fuction needs
    with open( ftbc , 'r') as f:                   # Opens the file parameter in the function as readable
        extract_n_write_OD_values(f)               # Extracts the OD value
        f.close()                                  # The file is closed
    file_order = LastIteration[0]                  # Sets the highest iteration value as an int
def merge_results():                               # Merges all the files specified while calling the script
...
def normalize_values():                            # Subtract the t0 values from all the values 
